...
class HologramRendererNew(context: BlockEntityRendererProvider.Context) extends BlockEntityRenderer[Hologram] with Callable[Int] with RemovalListener[BlockEntity, Int] {
  ...
  override def render(hologram: Hologram, partialTick: Float, poseStack: PoseStack, bufferSource: MultiBufferSource, packedLight: Int, packedOverlay: Int): Unit = {
    if (failed) {
      val fallbackRenderer = new HologramRendererFallback(context)
    ...
    // Calculate distance-based alpha for fade effect
    val playerPos = context.getBlockEntityRenderDispatcher.camera.getPosition
    val hologramPos = hologram.getBlockPos
    val playerDistSq = playerPos.distanceToSqr(hologramPos.getX + 0.5, hologramPos.getY + 0.5, hologramPos.getZ + 0.5)
    val maxDistSq = hologram.getMaxRenderDistanceSquared
    val fadeDistSq = hologram.getFadeStartDistanceSquared
    val alpha = 0.75f * (if (playerDistSq > fadeDistSq) math.max(0, 1 - ((playerDistSq - fadeDistSq) / (maxDistSq - fadeDistSq)).toFloat) else 1)
    ...
    val rotationQuat = new org.joml.Quaternionf().rotateXYZ(
      Math.toRadians(hologram.rotationAngle * hologram.rotationX).toFloat,
      Math.toRadians(hologram.rotationAngle * hologram.rotationY).toFloat,
      Math.toRadians(hologram.rotationAngle * hologram.rotationZ).toFloat
    )
    poseStack.mulPose(rotationQuat)
    
    // Apply rotation speed animation
    val worldTime = hologram.getLevel.getGameTime
    val animationAngle = hologram.rotationSpeed * ((worldTime % (360 * 20 - 1)) + partialTick) / 20f
    val speedRotationQuat = new org.joml.Quaternionf().rotateXYZ(
      Math.toRadians(animationAngle * hologram.rotationSpeedX).toFloat,
      Math.toRadians(animationAngle * hologram.rotationSpeedY).toFloat,
      Math.toRadians(animationAngle * hologram.rotationSpeedZ).toFloat
    )
    poseStack.mulPose(speedRotationQuat)

    poseStack.scale(1.001f, 1.001f, 1.001f) // Avoid z-fighting with other blocks.
    poseStack.translate(
    ...
    poseStack.scale(hologram.scale / 16f, hologram.scale / 16f, hologram.scale / 16f)

    RenderSystem.setShaderTexture(0, Textures.Model.HologramEffect)

    // Check if camera is inside the hologram for culling decisions
    val cameraPos = context.getBlockEntityRenderDispatcher.camera.getPosition
    val relativePos = cameraPos.subtract(hologramPos.getX + 0.5, hologramPos.getY + 0.5, hologramPos.getZ + 0.5)
    val sx = relativePos.x * hologram.scale
    val sy = -relativePos.y * hologram.scale
    val sz = relativePos.z * hologram.scale
    ...
    // Second pass: color with depth test
 ...
 */
object HologramRendererNew {
  def apply(context: BlockEntityRendererProvider.Context): HologramRendererNew = new HologramRendererNew(context)
}