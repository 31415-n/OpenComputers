    ...
    printEntity match {
      case t: tileentity.Print =>
        ...
        for (shape <- t.shapes if !Strings.isNullOrEmpty(shape.texture)) {
          ...
          faces ++= bakeQuads(makeBox(extendedAABB(bounds).min, extendedAABB(bounds).max), Array.fill(6)(texture), shape.tint.getOrElse(White)).toSeq
        }
  ...
  class ItemModel(val stack: ItemStack) extends SmartBlockModelBase {
    ...
    override def getQuads(state: BlockState, side: Direction, rand: net.minecraft.util.RandomSource): util.List[BakedQuad] = {
      ...
      if (shapes.isEmpty) {
        ...
        faces ++= bakeQuads(makeBox(extendedAABB(bounds).min, extendedAABB(bounds).max), Array.fill(6)(texture), Color.rgbValues(DyeColor.LIME)).toSeq
      }
