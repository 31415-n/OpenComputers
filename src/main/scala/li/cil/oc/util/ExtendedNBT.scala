...
object ExtendedNBT {
  ...
  implicit def toNbt(value: Boolean): ByteTag = ByteTag.valueOf(if (value) 1 else 0)
  ...
  implicit def toNbt(value: Map[String, _]): CompoundTag = {
    ...
    for ((key, value) <- value) value match {
      case value: Boolean => nbt.put(key, toNbt(value))
      case value: Byte => nbt.put(key, toNbt(value))
      case value: Short => nbt.put(key, toNbt(value))
      case value: Int => nbt.put(key, toNbt(value))
      case value: Long => nbt.put(key, toNbt(value))
      case value: Float => nbt.put(key, toNbt(value))
      case value: Double => nbt.put(key, toNbt(value))
      case value: Array[Byte] => nbt.put(key, toNbt(value))
      case value: Array[Int] => nbt.put(key, toNbt(value))
      case value: String => nbt.put(key, toNbt(value))
      case value: ItemStack => nbt.put(key, toNbt(value))
      case _ =>
    }
    nbt
  }
  ...
  implicit def extendNBTTagCompound(nbt: CompoundTag): ExtendedNBTTagCompound = new ExtendedNBTTagCompound(nbt)

  implicit def extendNBTTagList(nbt: ListTag): ExtendedNBTTagList = new ExtendedNBTTagList(nbt)
  ...
  class ExtendedNBTTagCompound(val nbt: CompoundTag) {
  ...
  class ExtendedNBTTagList(val nbt: ListTag) {
    ...
    def foreach[T <: Tag](f: T => Unit): Unit = {
      ...
      while (iterable.size() > 0) {
        f(iterable.remove(0).asInstanceOf[T])
      }
    }
    ...
    def map[T <: Tag, Value](f: T => Value): IndexedSeq[Value] = {
      ...
      while (iterable.size() > 0) {
        buffer += f(iterable.remove(0).asInstanceOf[T])
      }
      buffer
    }

    def toArray[T: ClassTag] = map((t: T) => t).toArray
  }
